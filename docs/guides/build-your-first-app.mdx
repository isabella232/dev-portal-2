...
At the top, we configure variables we'll need later. We also use [React effect hooks](https://reactjs.org/docs/hooks-effect.html) to update the URL and local storage on credential changes and listen for 'o' key presses to open the **Open Scene** dialog.
...
return (
  <Layout
    header={<OpenButton onOpenSceneClick={() => setDialogOpen(true)} />}
    main={
      ready && (
        <Viewer
          configEnv={Env}
...
- We return the application's components wrapped in a `Layout`.
- The header includes the **Open Scene** button.
- Next is the main application content:
  - If `ready`, we display the `Viewer` component.
  - With the `onSelect` callback, we set the properties state. Then, call a utility function similar to the [Render your first scene](./render-your-first-scene.mdx) guide's function to perform item selection.
- The right drawer receives the metadata properties for display.
- Finally, the **Open Scene** dialog. It's shown if the `dialogOpen` React state is `true`. We pass two callbacks:
  ...
  - `onConfirm` updates the credentials, then closes the dialog.

## Calling the Vertex API

Browse to [http://localhost:3000/api/files](http://localhost:3000/api/files). You should get a `401: Unauthorized` error. The application doesn't know your Vertex client ID and secret, so it cannot make API calls on your behalf. Let's fix that.

In your terminal, type `Ctrl+C` to stop the development server. NextJS has [built-in support for environment variables](https://nextjs.org/docs/basic-features/environment-variables), and the Vertex NextJS Starter comes with a `.env.local.template` file you can rename to make it easier for you to set them.

```shell
mv .env.local.template .env.local
```

Open `.env.local` and set the `VERTEX_CLIENT_ID` and `VERTEX_CLIENT_SECRET` to your Vertex client ID and secret. You can leave `NEXT_PUBLIC_VERTEX_ENV` blank. Since the environment variables don't start with `NEXT_PUBLIC_`, they are only exposed server-side, keeping your client secret safe.
...
Now, start the development server again and browse to [http://localhost:3000/api/files](http://localhost:3000/api/files). You should see a list of files you've uploaded!
...
We're using NextJS [API Routes](https://nextjs.org/docs/api-routes/introduction) and the `makeCall` helper function. Open [`src/lib/vertex-api.ts`](https://github.com/Vertexvis/vertex-nextjs-starter/blob/main/src/lib/vertex-api.ts) to see what that does.
