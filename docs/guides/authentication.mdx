...
Gain secure access to the Vertex APIs by exchanging OAuth 2.0 application credentials for an access token.

The flow is as follows:

- Your application makes a secure HTTP call, providing its application credentials. You receive an access token in response.
...
<Mermaid
  ...
  participant API as Vertex API
    CF->>+C: Authenticate
    C->>+API: Request token with application credentials
