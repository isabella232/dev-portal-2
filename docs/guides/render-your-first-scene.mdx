...
To render a scene, you'll need a stream key. Stream keys grant the Viewer component access to a specific scene. This is explained in more detail later, but for now, use our test key, `vpe5dKpDffDT8bLzhbP0S7Da2nN9-w-xgq6X`. Copy the following into the empty `<script>` tag.
